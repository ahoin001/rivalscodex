...
export function HeroStatsRail({ hero, activeForm }: HeroStatsRailProps) {
  return (
    <ClippedPanel
      tone="gold"
    ...
    >
      ...
      {activeForm.resource && (
        <HudSection
          title={`Resource: ${activeForm.resource.name}`}
          tone="primary"
          titleSize="sm"
          className="mt-4 bg-brand-gold-muted"
        >
          <p className="text-sm text-muted-foreground">{activeForm.resource.description}</p>
        </HudSection>
      )}
