...
export function HeroGuideBody({
  ...
  const renderComboBlock = (block: ComboBlock, index: number): ReactNode => {
    ...
    // Public read path uses CombosReaderPanel + ComboShowcaseCard.
    // ComboRouteCard is edit-only so reader bundles never pull the builder.
    const useRouteCard = comboEditMode && !!abilityLookup;
